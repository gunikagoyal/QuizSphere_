...
def get_information():
    ...
    if generate_button:
        ...
        with st.spinner('Generating Quiz...'):
            quiz_array = get_quiz_array(text, quiz_level, num_questions)
...
def download_quiz():
    ...
    # Loop through the quiz list and add questions and choices
    for question_data in quiz_list:
        question_number, question_text, *choices, correct_choice = question_data

        pdf.multi_cell(0, 10, f"Question {question_number}: {question_text}", align='L')
        pdf.ln(5)

        for index, choice in enumerate(choices, start=1):
            pdf.multi_cell(0, 10, f"{index}. {choice}", align='L')
        pdf.ln(5)
...
def main():
    ...
    st.markdown("QuizSphere is an AI powered Educational Quiz Generator that turns your educational video lectures into engaging MCQ based quizzes and provides automatic assessment. Learning has never been this fun!")
