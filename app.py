...
def show_buttons():
    ...
    if "show_buttons" in st.session_state and st.session_state.show_buttons:
        ...
        st.write("or")
        st.button("Home", on_click=return_to_home)
...
def download_quiz():
    ...
    pdf.cell(200, 10, txt="QuizSphere", ln=1, align='C')
...
def main():
    ...
    st.set_page_config(page_title="QuizSphere", page_icon="🎓")

    st.header("QuizSphere 🎓")
    st.markdown("QuizSphere is an AI powered Educational Quiz Generator to transform your educational video lectures into interactive MCQ based quizzes and provides automatic assessment. Experience a new way of learning!")
    st.write("It utilizes the transcription feature of **AssemblyAI** and transcribes a video such as a video lecture from youtube and then gives the transcribed text to GPT-4 Model from **Clarifai** which generates a quiz for the user to take and assess their knowledge in an easy and user-friendly manner.")
    st.write("")
